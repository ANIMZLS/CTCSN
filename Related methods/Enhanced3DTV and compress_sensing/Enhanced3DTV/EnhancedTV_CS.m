...
s    = sizeD(3);
%% FFT setting
Eny_x   = ( abs(psf2otf([+1; -1], [h,w,s])) ).^2  ;
Eny_y   = ( abs(psf2otf([+1, -1], [h,w,s])) ).^2  ;
Eny_z   = ( abs(psf2otf([+1, -1], [w,s,h])) ).^2  ;
...
%[mp,sm,er,sa,rm] = msqia2(reshape(x1,[h,w,s])/255,opts.trX/255);
%fprintf('mp = %f, sm = %f, er =%f, sa =%f, rm =%f.\n',mp,sm,er,sa,rm);
...
%     x        = real( ifftn( fftn(numer1) ./ (mu*determ + mu) ) );
    ...
    L     = reshape(x1,sizeD);
%    [mp,sm,er,sa,rm] = msqia2(L/255,opts.trX/255);
%    fprintf('The clean image: mp = %f, sm = %f, er =%f, sa =%f, rm =%f.\n',mp,sm,er,sa,rm);
%    [mp,sm,er,sa,rm] = msqia2(x_ten/255,opts.trX/255);
%    fprintf('The full image: mp = %f, sm = %f, er =%f, sa =%f, rm =%f.\n',mp,sm,er,sa,rm);
